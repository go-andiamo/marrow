...
func NewApi(cfg config.Config, repo repository.Repository, msgp cloud.MessagePublisher) Api {
	return &api{
		appCfg: cfg,
		cfg:    cfg.Api,
		repo:   repo,
...
func (a *api) Start() {
	...
	if err := http.ListenAndServe(fmt.Sprintf(":%s", a.cfg.Port), router); err != nil {
...
func (a *api) postPets(w http.ResponseWriter, r *http.Request) {
	...
	if err = json.NewDecoder(r.Body).Decode(&add); err == nil {
		...
		if result, err = a.repo.AddPet(r.Context(), add); err == nil {
			_ = a.msgp.PublishMessage(r.Context(), a.appCfg.Topics.Pets, fmt.Sprintf("created pet: %v", result))

...
func TestWithInit_Mocked(t *testing.T) {
	...
	_, ok := init.called["AddSupportingImage:artemis"]
	assert.True(t, ok)
	...
	assert.Equal(t, defaultBrokerPort, img.Port())
	assert.NotEqual(t, defaultBrokerPort, img.MappedPort())
	assert.Equal(t, "artemis", img.Username())
	assert.Equal(t, "artemis", img.Password())

...
func TestWithInit_Mocked(t *testing.T) {
	...
	assert.True(t, ok)
	_, ok = init.called["AddSupportingImage:postgres"]
	assert.True(t, ok)
	assert.Len(t, init.images, 1)
	img, ok := init.images["postgres"]
	assert.True(t, ok)
	assert.Equal(t, "postgres", img.Name())
	...
	assert.Equal(t, defaultUsername, img.Username())
	assert.Equal(t, defaultPassword, img.Password())
